...
- 默认：`None`
- 说明：每日表情包制作次数限制的提示配置项
  - `result`
    - 类型：`str | None`
    - 默认：`None`
    - 说明：超限时返回文案；为空时回退到默认提示 `今日表情包制作次数已达上限，请明天再试`
  - `mode / max_count / group_max_count`
    - 说明：旧版静态限额字段，已废弃且不再参与运行时判定，仅保留为兼容旧配置
- `memes_daily_limit` 在 `.env` 文件中的设置示例如下：

```
memes_daily_limit='{"result":"今日表情包额度已用完，请明天再试"}'
```

其中：

- 实际的次数上限通过群内管理命令动态配置，而不是通过 `.env` 静态配置
- 默认所有群聊和私聊都不计数、不限额
- 同一用户在同一会话范围内超限时，同一天仅会收到一次提示，后续同日请求会静默拦截
...
“超级用户” 和 “管理员” 可以在群聊中配置当前群内每个用户的每日表情包制作次数

- 发送 `表情包次数/表情次数/meme次数/memes次数 + 正整数` 设置当前群的每日次数限制
- 发送 `表情包次数 -1` 关闭当前群的次数限制
- 发送 `表情包次数` 查询当前群的次数限制

“超级用户” 可在与 bot 的私聊中配置“普通用户私聊 bot 时”的全局每日次数限制

- 发送 `表情包次数 + 正整数` 设置普通用户私聊的全局每日次数限制
- 发送 `表情包次数 -1` 关闭普通用户私聊的次数限制
- 发送 `表情包次数` 查询当前普通用户私聊的次数限制

> [!NOTE]
>
> 默认未启用限制的群聊/私聊不会计数，因此统计图仅覆盖启用过次数限制并成功制作的调用记录
